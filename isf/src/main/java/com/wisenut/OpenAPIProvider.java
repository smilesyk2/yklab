...
public class OpenAPIProvider {
	...
	// 1-1. Model + SF-1 결과 조합. 날짜 조건 없음. 전체 컬렉션 대상.
	public String getRecommendedContentsInfoWithSF1Result(String article, int start, int pageno) throws Exception{
		return getRecommendedContentsInfoWithSF1Result(article, "all", start, pageno, "", "");
	}
	
	// 1-2. Model + SF-1 결과 조합. 날짜 조건 없음. 특정 컬렉션 대상.
	public String getRecommendedContentsInfoWithSF1Result(String article, String targetCollection, int start, int pageno) throws Exception{
		return getRecommendedContentsInfoWithSF1Result(article, targetCollection, start, pageno, "", "");
	}
	
	// 1-3. Model + SF-1 결과 조합. 날짜 조건 없음. 특정 컬렉션 대상.
	...
	// 1-4. Model + SF-1 결과 조합. 날짜 조건 있음. 특정 컬렉션 대상
	public String getRecommendedContentsInfoWithSF1Result(String articleOrWord, String targetCollection, int start, int pageno, String startDate, String endDate) throws Exception{
		...
		List<Pair<Integer>> keywordList = teaWorker.getMainKeywordsPair(articleOrWord, start, pageno);
        ...
        if(targetCollection.equals("all")){
        	for(String col : collections){
                ...
                List<Pair<Double>> docidList = teaWorker.getRecommendedContentsPair(articleOrWord, searchWorker.getDocidMap().get(col), pageno);
        	...
        	// 검색한 결과와 tea의 similarDoc 결과를 조합. 검색 결과 중 기사(article)의 결과만 리스트로 제공
            List<Pair<Double>> docidList = teaWorker.getRecommendedContentsPair(articleOrWord, searchWorker.getDocidMap().get(targetCollection), pageno);
            ArrayList<HashMap<String,String>> resultList = new ArrayList<HashMap<String,String>>();
	...
	public static void main(String[] args){
		...
		try {
			...
			System.out.println("#3-1. SF-1 검색 결과를 조합한 연관기사 추천. 전체 컬렉션. 날짜 조건 X : " + strResultData);
			
			strResultData = provider.getRecommendedContentsInfoWithSF1Result(article, "image", 0, 10);
			System.out.println("#3-2. SF-1 검색 결과를 조합한 연관기사 추천. 특정 컬렉션. 날짜 조건 X : " + strResultData);
			
			strResultData = provider.getRecommendedContentsInfoWithSF1Result(article, 0, 10, "20140101", "20141231");
			System.out.println("#3-3. SF-1 검색 결과를 조합한 연관기사 추천. 전체 컬렉션. 날짜 조건 O : " + strResultData);
			
			strResultData = provider.getRecommendedContentsInfoWithSF1Result(article, "article", 0, 10, "20140101", "20141231");
			System.out.println("#3-4. SF-1 검색 결과를 조합한 연관기사 추천. 특정 컬렉션. 날짜 조건 O : " + strResultData);
